...
im  = imread('D:\CNN\SRCNN\2015SRCNN\SRCNN_v11\SRCNN\Set5\butterfly_GT.bmp');
...
im_h= imread('D:\CNN\SRCNN\2015SRCNN\SRCNN_v11\SRCNN\b_694070.bmp');%只需要改亮度图像来源
%im_h111 = imread('D:\CNN\SRCNN\2015SRCNN\SRCNN_v11\SRCNN\b_694070.bmp');%只需要改亮度图像来源
%im_h11=uint8(zeros(256));
%im_h11(2:256,2:256)=im_h111;
%im_h=im_h11;
%im_h=im_h(3:354,3:242);
...
psnr_bic = compute_psnr(im_gnd,im_b);
psnr_srcnn = compute_psnr(im_gnd,im_h);
SSIM_bic  = ssim_index_vdsr(im_gnd, im_b, [0.01 0.03])
SSIM_SRCNN = ssim_index_vdsr(im_gnd, im_h, [0.01 0.03])
...
imwrite(im_h2, ['sdsr2' '.bmp']);
imwrite(im_h4, ['BICUBIC pepper19 color Reconstruction' '.bmp']);
